...
import com.yuning.ui.ArcMenu.OnCallbackListener;
...
public class ArcLayout extends ViewGroup {
    ...
    private void init() {
    	...
    	mChildPadding *= density;
    	mLayoutPadding *= density;
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        final int centerX = getCenterX();//getWidth() / 2;
        final int centerY = getCenterY();//getHeight() / 2;
    ...
    private void bindChildAnimation(final View child, final int index, final long duration) {
        ...
        Interpolator interpolator = mExpanded ? new AccelerateInterpolator() : new OvershootInterpolator(0.0f);
    ...
    public void switchState(final boolean showAnimation) {
        if (showAnimation) {
            final int childCount = getChildCount();
            for (int i = 0; i < childCount; i++) {
                bindChildAnimation(getChildAt(i), i, ArcMenu.ANIMATION_DURATON);
            }
        }

        mAnimating = showAnimation;
        mExpanded = !mExpanded;
	...
	public void setHintView(View hint) {
		mHintView = hint;
	}
	
	public int getCenterX() {
		if(mHintView != null) {
			return mHintView.getWidth() / 2;
		}
		return getChildSize() / 2;
	}
	
	public int getCenterY() {
		if(mHintView != null) {
			return getHeight() - mHintView.getHeight() / 2;
		}
		return getHeight() - getChildSize() / 2;
	}
